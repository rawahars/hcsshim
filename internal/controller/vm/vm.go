...
func (c *Manager) CreateVM(ctx context.Context, opts *CreateOptions) error {
	...
	if c.vmState != StateNotCreated {
		return fmt.Errorf("cannot create VM: VM is already in state %s", c.vmState)
	}
...
func (c *Manager) StartVM(ctx context.Context, opts *StartOptions) (err error) {
	...
	if c.vmState != StateCreated {
		return fmt.Errorf("cannot start VM: VM is in state %s", c.vmState)
	}

	defer func() {
		if err != nil {
			// If starting the VM fails, we transition to Invalid state to prevent any further operations on the VM.
			// The VM can be called by invoking TerminateVM API.
...
func (c *Manager) waitForVMExit(ctx context.Context) {
	...
	_ = c.uvm.Wait(ctx)
	// Once the VM has exited, record the Terminated state under the mutex.
	c.mu.Lock()
...
func (c *Manager) ExecIntoHost(ctx context.Context, request *shimdiag.ExecProcessRequest) (int, error) {
	...
	if c.vmState != StateRunning {
		c.mu.Unlock()
		return -1, fmt.Errorf("cannot exec into VM: VM is not in state %s", c.vmState)
	}
	c.mu.Unlock()
...
func (c *Manager) DumpStacks(ctx context.Context) (string, error) {
	...
	if c.vmState != StateRunning {
		c.mu.Unlock()
		return "", fmt.Errorf("cannot dump stacks: VM is not in state %s", c.vmState)
	}
	c.mu.Unlock()
...
func (c *Manager) Wait(ctx context.Context) error {
	...
	// Validate that the VM is in at least created state and can be waited on.
	c.mu.Lock()
	if c.vmState != StateCreated {
		c.mu.Unlock()
		return fmt.Errorf("cannot wait on VM: VM is in state %s", c.vmState)
	}
	c.mu.Unlock()
...
func (c *Manager) Stats(ctx context.Context) (*stats.VirtualMachineStatistics, error) {
	...
	if c.vmState != StateRunning {
		return nil, fmt.Errorf("cannot get stats: VM is in state %s", c.vmState)
	}
...
func (c *Manager) TerminateVM(ctx context.Context) (err error) {
	...
	err = c.uvm.Close(ctx)
	if err != nil {
		// Set state to invalid so that we do not perform any active operation on the VM.
		c.vmState = StateInvalid
...
func (c *Manager) ExitStatus() (*ExitStatus, error) {
	...
	if c.vmState != StateTerminated {
		return nil, fmt.Errorf("cannot get stopped status: VM is in state %s", c.vmState)
	}
