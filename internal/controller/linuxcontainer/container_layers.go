...
func (c *Controller) allocateLayers(
	...
	for _, roLayer := range lcowLayers.Layers {
		...
		hostPath, err := fs.ResolvePath(roLayer.VHDPath)
		if err != nil {
	...
	scratchHostPath, err := fs.ResolvePath(lcowLayers.ScratchVHDPath)
	if err != nil {
	...
	if err = wclayer.GrantVmAccess(ctx, c.vmID, scratchHostPath); err != nil {
	...
	if err = c.guestMgr.AddLCOWCombinedLayers(ctx, guestresource.LCOWCombinedLayers{
