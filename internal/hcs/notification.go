...
func notificationHandler(eventPtr uintptr, ctx uintptr) uintptr {
	...
	// DEBUG: bump notification logging to Info so we can correlate event
	// timing in containerd.log without enabling shim-wide debug.
	logrus.WithFields(fields).Infof("HCS %s notification", source)
	return 0
}
