...
const (
	...
	// parentCgroupPathPrefix is the prefix for cgroup paths for pods.
	// Individual pods are created under this path.
	// Example: /pods/<SANDBOX_ID>
	parentCgroupPathPrefix = "/pods"
)

// uvmPod represents a pod within the UVM/Sandbox.
type uvmPod struct {
	sandboxID     string
	spec          *specs.Spec
	cgroupPath    string
...
// Host is the structure tracking all UVM host state including all pods and containers
// and processes.
type Host struct {
...
func (h *Host) RemoveContainer(id string) {
	...
	// For CRI workloads, we need to update the pod structure as well.
	if isCRI {
		...
		// For a sandbox container, we will delete the cgroup and
		// delete the pod.
		if criType == "sandbox" {
			h.teardownSandboxPod(pod)
		}
	}
...
func setupSandboxLogDir(sandboxID string) error {
	mountPath := specGuest.SandboxLogsDir(sandboxID)
	if err := mkdirAllModePerm(mountPath); err != nil {
		return errors.Wrapf(err, "failed to create sandbox logs dir in sandbox %v", sandboxID)
	}
	return nil
}
...
func (h *Host) CreateContainer(ctx context.Context, id string, settings *prot.VMHostedContainerSettingsV2) (_ *Container, err error) {
	...
	if isCRI {
		switch criType {
		case "sandbox":
			_, exists := h.getPod(sandboxID)
			if exists {
				return nil, errors.Errorf("sandbox container %s already exists", id)
			}
			...
			// Create the pod within the UVM for the given sandbox.
			pod, err := h.initSandboxPod(sandboxID, settings.OCISpecification)
			if err != nil {
				return nil, errors.Wrapf(err, "failed to create pod %s in the uvm", sandboxID)
			}
			// On failure, clean up the pod.
			defer func() {
				if err != nil {
					h.teardownSandboxPod(pod)
				}
			...
			err = setupSandboxContainerSpec(ctx, pod, settings.OCISpecification)
			if err != nil {
				return nil, err
			}
			defer func() {
			...
			// Set the mounts for the specific pod within the UVM.
			...
			// Create the log directory for the sandbox.
		...
		case "container":
			...
			// Check that the sandbox exists for the container.
			pod, exists := h.getPod(sandboxID)
			if !exists {
				return nil, errors.Errorf("sandbox %s for container %s does not exist", sid, id)
			}

			if err = setupWorkloadContainerSpec(ctx, pod, id, settings.OCISpecification, settings.OCIBundlePath); err != nil {
				return nil, err
			}
		...
		// Set the sandboxID corresponding to the container.
...
func (h *Host) getPod(sandboxID string) (*uvmPod, bool) {
	h.podsMutex.Lock()
	defer h.podsMutex.Unlock()

	pod, exists := h.pods[sandboxID]
	return pod, exists
}

// initSandboxPod creates a pod structure in the UVM to tr
func (h *Host) initSandboxPod(sid string, pSpec *specs.Spec) (*uvmPod, error) {
	h.podsMutex.Lock()
	defer h.podsMutex.Unlock()
	...
	if _, exists := h.pods[sid]; exists {
		logrus.WithFields(logrus.Fields{
			"sandboxID": sid,
		}).Errorf("Pod already exists")

		return nil, fmt.Errorf("pod with id %s already exists", sid)
	}

	// Create the cgroup path for this pod under the parent cgroup.
	cgroupPath := path.Join(parentCgroupPathPrefix, sid)
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to create cgroup for pod %s", sid)
	}
	...
	// Add the pod to the UVM host's pod map.
	h.pods[sid] = pod

	logrus.WithFields(logrus.Fields{
		"sandboxID":  sid,
		"cgroupPath": cgroupPath,
		"createdAt":  pod.createdAt.String(),
	}).Info("pod created successfully")

	return pod, nil
}

// teardownSandboxPod removes the pod from the UVM host's pod map and deletes its cgroup.
func (h *Host) teardownSandboxPod(pod *uvmPod) {
	h.podsMutex.Lock()
	defer h.podsMutex.Unlock()
	...
	if err := pod.cgroupControl.Delete(); err != nil {
		...
		).Warnf("Failed to delete pod cgroup")
	}

	delete(h.pods, pod.sandboxID)
}
