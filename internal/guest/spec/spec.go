...
var (
	// devicesHostDevicesFn is a variable to allow mocking in tests.
	devicesHostDevicesFn = devices.HostDevices
	// devicesDeviceFromPathFn is a variable to allow mocking in tests.
	devicesDeviceFromPathFn = devices.DeviceFromPath
)
...
// This root dir considers the UVM supporting multiple pods.
...
// specific for the given sandbox/pod.
...
func ApplyAnnotationsToSpec(ctx context.Context, spec *oci.Spec) error {
	...
	if privileged {
		...
		hostDevices, err := devicesHostDevicesFn()
		if err != nil {
			return err
		}
		...
		for _, ld := range tempLinuxDevices {
			hostDevice, err := devicesDeviceFromPathFn(ld.Path, "rwm")
			if err != nil {
				return err
			}
