...
func getServeCommand(shim Shim) cli.Command {
	var serveCommand = cli.Command{
		...
		Action: func(ctx *cli.Context) error {
			...
			// V2 API and the Sandbox V1 API.
			//
			// The activation model for this shim is as follows:
			//
			// The public invocation of `shim start` is called which serves a
			// new shim. serve execs this entry point `shim serve`.
			...
			case runhcsopts.Options_NPIPE:
				...
				logl, err := winio.ListenPipe(fmt.Sprintf(logAddrFmt, shimCtx.namespace, shimCtx.id), nil)
				if err != nil {
					return err
				}
				defer logl.Close()
			...
			ttrpcEventPublisher, err := newEventPublisher(ttrpcAddress, shimCtx.namespace)
			if err != nil {
				return err
			}
			defer func() {
			...
			// Register the services on the ttrpc server.
			err = shim.RegisterServices(ctx, s, ttrpcEventPublisher)
			if err != nil {
				return err
			}
